================================================================================
       Windows APPLICATION: VBIEExplorerBar Overview                        
===============================================================================
/////////////////////////////////////////////////////////////////////////////
Summary:
...
For 32bit IE on x86 or x64 OS, install VBIEExplorerBarSetup(x86).msi, the output
of the VBIEExplorerBarSetup(x86) setup project.

For 64bit IE on x64 OS, install VBIEExplorerBarSetup(x64).msi outputted by the 
VBIEExplorerBarSetup(x64) setup project.

B. Removal

For 32bit IE on x86 or x64 OS, uninstall VBIEExplorerBarSetup(x86).msi, the 
output of the VBIEExplorerBarSetup(x86) setup project. 

For 64bit IE on x64 OS, uninstall VBIEExplorerBarSetup(x64).msi, the output of
the VBIEExplorerBarSetup(x64) setup project.
...
Step1. Open this project in VS2008 and set the platform of the solution to x86. Make
       sure that the projects VBIEExplorerBar and VBIEExplorerBarSetup(x86)
	   ...
	   NOTE: If you want to run this sample in 64bit IE, set the platform to x64 and 
	         select VBIEExplorerBar and VBIEExplorerBarSetup(x64) to build.
        
Step2. Build the solution.
 
Step3. Right click the project VBIEExplorerBarSetup(x86) in Solution Explorer, 
       and choose "Install".
...
In Visual Studio 2008, create a Visual VB / Windows / Class Library project 
named "VBIEExplorerBar". 
...
D. Deploying the IE Explorer Bar with a setup project.
  ...
  VBIEExplorerBarSetup(x86). Click OK to create the project. 
  ...
  In the Add Project Output Group dialog box, VBIEExplorerBar will  
  be displayed in the Project list. Select Primary Output and click OK. VS
  will detect the dependencies of the VBIEExplorerBar, including .NET
  ...
  VBIEExplorerBar. This adds the custom actions that we defined 
  in IEExplorerBarInstaller of VBIEExplorerBar. 
  ...
  must create a new setup project (e.g. VBIEExplorerBarSetup(x64) 
  ...
  file in the VBIEExplorerBarSetup(x64) project folder. To 
  configure the script to run in the post-build event, you select the 
  VBIEExplorerBarSetup(x64) project in Solution Explorer, and 
