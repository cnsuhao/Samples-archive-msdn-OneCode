...
$serviceApp = Get-SPServiceApplication | where { $_.GetType().FullName -eq "CustomService.CustomServiceApplication" -and $_.Name -eq "CustomServiceApp1" }
...
$serviceAppProxy = Get-SPServiceApplicationProxy | where { $_.GetType().FullName -eq "CustomService.CustomServiceApplicationProxy" -and $_.Name -eq "CustomServiceAppProxy1" }
...
$localServiceInstance = Get-SPServiceInstance -Server $env:computername | where { $_.GetType().FullName -eq "CustomService.CustomServiceInstance" -and $_.Name -eq "" }
