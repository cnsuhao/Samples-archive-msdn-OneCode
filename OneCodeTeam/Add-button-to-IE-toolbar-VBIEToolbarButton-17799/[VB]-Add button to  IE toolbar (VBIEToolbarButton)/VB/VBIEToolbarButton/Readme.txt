================================================================================
       Windows APPLICATION: VBIEToolbarButton Overview                        
===============================================================================
/////////////////////////////////////////////////////////////////////////////
Summary:
...
For 32bit IE on x86 or x64 OS, run 'Visual Studio Command Prompt (2008)' in the 
Microsoft Visual Studio 2008 \ Visual Studio Tools menu as administrator. For 64bit
IE on x64 OS, run Visual Studio x64 Win64 Command Prompt (2008).

Navigate to the folder that contains the build result VBIEToolbarButton.dll and 
enter the command:

    Regasm.exe VBIEToolbarButton.dll /codebase
...
enter the command:
    ...
    Regasm.exe VBIEToolbarButton.dll /unregister
...
For 32bit IE on x86 or x64 OS, install VBIEToolbarButtonSetup(x86).msi, the output
of the VBIEToolbarButtonSetup(x86) setup project.

For 64bit IE on x64 OS, install VBIEToolbarButtonSetup(x64).msi outputted by the 
VBIEToolbarButtonSetup(x64) setup project.

B. Removal

For 32bit IE on x86 or x64 OS, uninstall VBIEToolbarButtonSetup(x86).msi, the 
output of the VBIEToolbarButtonSetup(x86) setup project. 

For 64bit IE on x64 OS, uninstall VBIEToolbarButtonSetup(x64).msi, the output of
the VBIEToolbarButtonSetup(x64) setup project.
...
Step1. Open this project in VS2008 and set the platform of the solution to x86. Make
       sure that the projects VBIEToolbarButton and VBIEToolbarButtonSetup(x86)
	   ...
	   NOTE: If you want to use this Toolbar button in 64bit IE, set the platform to x64 and 
	         select VBIEToolbarButton and VBIEToolbarButtonSetup(x64) to build.
        
Step2. Build the solution.
 
Step3. Right click the project VBIEToolbarButtonSetup(x86) in Solution Explorer, 
       and choose "Install".
...
In Visual Studio 2008, create a Visual Basic / Windows / Class Library project 
named "VBIEToolbarButton". 
...
The NotePadToolbarButton.vb file defines the Toolbar button. 
...
D. Deploying the Toolbar Button with a setup project.
  ...
  VBIEToolbarButtonSetup(x86). Click OK to create the project. 
  ...
  In the Add Project Output Group dialog box, VBIEToolbarButton will  
  be displayed in the Project list. Select Primary Output and click OK. VS
  will detect the dependencies of the VBIEToolbarButton, including .NET
  ...
  VBIEToolbarButton. This adds the custom actions that we defined 
  in BHOInstaller of VBIEToolbarButton. 
  ...
  must create a new setup project (e.g. VBIEToolbarButtonSetup(x64) 
  ...
  file in the VBIEToolbarButtonSetup(x64) project folder. To 
  configure the script to run in the post-build event, you select the 
  VBIEToolbarButtonSetup(x64) project in Solution Explorer, and 
